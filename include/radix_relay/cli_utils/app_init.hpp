...
inline auto configure_logging(const CliArgs &args) -> void
{
  if (args.verbose) { spdlog::set_level(spdlog::level::debug); }
}

inline auto get_node_fingerprint() -> std::string { return std::string(radix_relay::get_node_identity_fingerprint()); }

inline auto print_app_banner(const AppState &state) -> void
{

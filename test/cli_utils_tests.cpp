...
TEST_CASE("cli_args default values", "[cli_utils][cli_parser]")
{
  ...
  CHECK(args.identity_path == "~/.radix/identity.db");
  CHECK(args.mode == "hybrid");
  CHECK(args.verbose == false);
  CHECK(args.show_version == false);
  CHECK(args.send_parsed == false);
  CHECK(args.peers_parsed == false);
  CHECK(args.status_parsed == false);
  CHECK(args.send_recipient.empty());
  CHECK(args.send_message.empty());
}
...
TEST_CASE("validate_cli_args validates mode", "[cli_utils][cli_parser]")
{
  SECTION("valid modes pass validation")
  {
    ...
    args.mode = "internet";
    CHECK(radix_relay::cli_utils::validate_cli_args(args) == true);
  ...
  SECTION("invalid modes fail validation")
  {
    ...
    args.mode = "invalid";
    CHECK(radix_relay::cli_utils::validate_cli_args(args) == false);
    ...
    std::filesystem::path(radix_relay::platform::get_temp_directory()) / "test_execute_cli_version.db";
  {
    ...
    auto connection_monitor_queue =
      std::make_shared<radix_relay::async::async_queue<radix_relay::core::events::connection_monitor::in_t>>(
        io_context);
    auto command_handler = radix_relay::core::make_command_handler(
      bridge_wrapper, output_queue, transport_queue, session_queue, connection_monitor_queue);

    radix_relay::cli_utils::cli_args args;
    args.show_version = true;

    CHECK(radix_relay::cli_utils::execute_cli_command(args, command_handler) == true);
  }
...
TEST_CASE("execute_cli_command returns false for no commands", "[cli_utils][app_init]")
{
  const auto db_path = std::filesystem::path(radix_relay::platform::get_temp_directory()) / "test_execute_cli_false.db";
  {
    ...
    CHECK(radix_relay::cli_utils::execute_cli_command(args, command_handler) == false);
  }
  std::ignore = std::filesystem::remove(db_path);
}
...
TEST_CASE("app_state construction", "[cli_utils][app_init]")
{
  ...
  CHECK(state.node_fingerprint == "RDX:test123");
  CHECK(state.mode == "hybrid");
  CHECK(state.identity_path == "~/.radix/test.key");
}

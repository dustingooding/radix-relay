...
class test_double_ble_stream
{
public:
  using connection_params_t = radix_relay::transport::ble_connection_params;
  ...
  explicit test_double_ble_stream(const std::shared_ptr<boost::asio::io_context> &io_context) : io_context_(io_context)
  {}
  ...
  auto set_mtu(std::size_t mtu) -> void { mtu_ = mtu; }
  ...
  [[nodiscard]] auto get_mtu() const -> std::size_t { return mtu_; }
  ...
  auto async_connect(radix_relay::transport::ble_connection_params params,
    std::function<void(const boost::system::error_code &, std::size_t)> handler) -> void
  {
    connections_.push_back({ std::string(params.device_address),
      std::string(params.service_uuid),
      std::string(params.characteristic_uuid) });
...
static_assert(radix_relay::concepts::transport_stream<test_double_ble_stream>);
